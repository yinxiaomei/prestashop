	...
	<div class="row-fluid">
		...
		<div id="pb-right-column" class="span6">
			<div class="row-fuild"> 
				...
				<!-- thumbnails -->
				<div id="views_block" class="{if isset($images) && count($images) < 2}hidden{/if} span3">
					{if isset($images) && count($images) > 3}<span class="view_scroll_spacer "><a id="view_scroll_left" class="hidden icon-angle-up" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}">&nbsp;</a></span>{/if}
					<div id="thumbs_list">
				...
				{if $have_image}
					<span id="view_full_size">
						<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html'}"{if $jqZoomEnabled && $have_image} class="jqzoom"{/if} title="{$product->name|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}" id="bigpic" />
				...
				<div class="block_content">
					...
					{/if}
						...
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<div class="p-item span6 ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
								<div class="product-container">
									...
									<div class="right_block">
										<p class="s_title_block">
											<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|escape|truncate:30:'...':true}</a>
										</p>
										<div class="product_desc">
											<div class="block_description">
												<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{l s='More'}" class="product_description">{$accessory.description_short|strip_tags|truncate:50:'...'}</a>
											</div>
					...
					{if ($accessory@iteration%2==0||$smarty.foreach.accessories_list.last)}
						</div>
					{/if}
