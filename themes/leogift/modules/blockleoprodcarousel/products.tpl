...
<div class=" carousel slide" id="{$tabname}">
	...
	{foreach from=$mproducts item=products name=mypLoop}
		<div class="item {if $smarty.foreach.mypLoop.first}active{/if}">
				{foreach from=$products item=product key=number name=products}
				{if $product@iteration%$columnspage==1&&$columnspage>1}
				  <div class="row-fluid">
				{/if}
				<div  class="p-item {if $number<3}span4{elseif $number>3 && $number<6}span6{/if} {if $number==3 || $number==6}hideone{/if} product_block ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}p-item{/if} clearfix">
					<div class="product-container">
						<div class="center_block">
							<a href="{$product.link}" title="{$product.name|escape:html:'UTF-8'}" class="product_image"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')}" alt="{$product.name|escape:html:'UTF-8'}" />{if isset($product.new) && $product.new == 1}<span class="new">{l s='Hot' mod='blockleoprodcarousel'}</span>{/if}</a>
							...
							<a class="lnk_more button comparator" href="{$product.link}" title="{l s='View' mod='blockleoprodcarousel'}"><i class="icon-file">&nbsp;</i>{l s='View' mod='blockleoprodcarousel'}</a>
  						</div>
