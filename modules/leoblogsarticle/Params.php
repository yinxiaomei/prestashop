    ...
    */
	public function getFolderList( $path ) {
        ...
        public function getBlogsCategory(){
            ...
            $query = '
		SELECT c.id_lofblogs_category as id_category, cl.name as text, c.level_depth as level, c.id_parent
		...
		WHERE cl.id_lang = ' . intval($id_lang) ;
            $allCat = Db::getInstance()->ExecuteS($query);
        ...
        public function blogCategoryTag( $name, $value, $title, $attr='', $liAtrr="", $ulAttr = "", $tooltip="", $textAllCat = ""){
	//	echo '<pre>'.print_r($value,1 ); die;
            $children  = $this->getBlogsCategory();
            ...
            foreach($list as $cat){
                ...
                $options  .= '<option value="'.$cat["id_category"].'" '.$isSelected.'>---| '.$cat["tree"].$cat["text"].'</option>';                                       
            }
