...
class clipart extends Module
{
	...
	public function __construct()
	{
		...
		$this->version  = '1.2';
		$this->author 	= 'xiaomei.yin';
		$this->need_instance = 0;
		$this->secure_key = Tools::encrypt($this->name);
		
		parent::__construct();
                $controller = Dispatcher::getInstance()->getController();
                if($controller=="AdminModules") $this->_includeFile();

		$this->displayName = $this->l('clipart');
        ...
        public function _includeFile(){
            if( !class_exists("LeoBaseSourcemini") ){
                include_once(_PS_MODULE_DIR_.'clipart/libs/BaseSource.php');	
            }
            include_once(_PS_MODULE_DIR_.'clipart/libs/Slideshow.php');
            include_once(_PS_MODULE_DIR_.'clipart/libs/Params.php');
            if (!class_exists('PhpThumbFactory')) {
                require  _PS_MODULE_DIR_.'clipart/libs/phpthumb/ThumbLib.inc.php';
            }
		$this->_prepareForm();
	...
	{
		/* Adds Module */
		if (parent::install() AND $this->registerHook('rightColumn') AND $this->registerHook('header'))
		{
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new clipartControl($slidemini['id_slide']);
			$to_del->delete();
		}
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slidemini = new clipartControl((int)Tools::getValue('id_slide'));
	...
	private function _postProcess()
	{
		...
		{
			$slidemini = new clipartControl((int)Tools::getValue('delete_id_slide'));
	...
	{
            if (!$this->isCached('clipart.tpl', $this->getCacheId()))
            {
		$this->_includeFile();
		$slideminir = array();
		...
		$slideminis = LeoBaseSourcemini::getSource( $source )
			->setModuleName('clipart', $path, $site_url )
		...
		$this->smarty->assign('clipart_slides', $slideminis);
		$this->smarty->assign('clipart', $slideminir);
		$this->smarty->assign('clipart_modid', $this->id);
            }
		return true;
	}
	...
	public function hookDisplayTop()
	{
		...
		return $this->display(__FILE__, 'clipart.tpl', $this->getCacheId());
	}
        public function clearCache()
        {
                $this->_clearCache('clipart.tpl');
        }
