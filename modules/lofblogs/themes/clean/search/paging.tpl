	...
	{if $n < $nb_products}
		...
		{section name=pagination start=$p-1 loop=$p+2 step=1}
			...
			{elseif $smarty.section.pagination.index > 0 && $nb_products+$n > ($smarty.section.pagination.index)*($n)}
				<li><a href="{$link_paging}&p={$smarty.section.pagination.index}&n={$n}">{$smarty.section.pagination.index|escape:'htmlall':'UTF-8'}</a></li>
			{/if}
		{/section}
		{if $pages_nb-$p > 1}
			{if $pages_nb-$p > 2}
				<li class="truncate">...</li>
			{/if}
			<li><a href="{$link_paging}&p={$pages_nb}&n={$n}">{$pages_nb}</a></li>
		{/if}
		{if $nb_products > $p * $n}
			{assign var='p_next' value=$p+1}
			<li id="pagination_next"><a href="{$link_paging}&p={$p_next}&n={$n}">{l s='Next' mod='loyalty'}&nbsp;&raquo;</a></li>
		{else}
