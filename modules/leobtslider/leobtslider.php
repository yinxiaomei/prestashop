...
class leobtslider extends Module
{
	...
	private function _displayAddForm()
	{
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" width="'.($this->getParams()->get('imgwidth',690)/2).'" height="'.($this->getParams()->get('imgheight',360)/2).'" alt=""/>';
			$this->_html .= '</div>';
		}
