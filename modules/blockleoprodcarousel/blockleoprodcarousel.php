...
//         if (!$this->isCached('blockleoprodcarousel.tpl', $this->getCacheId())) {
            ...
            $rsproducts = $this->fileterProducts($products);
            //----------------  
...
//         }
        ...
        //$this->context->controller->addJS(($this->_path) . 'blockleoprodcarousel.js', 'all');
    ...
    public function fileterProducts($products)
    {
    	$result_products = array();
    	$context = Context::getContext();
    	 
    	if(isset($context->cookie->showProducts)){
    		$len = count($products);
    		$factor = $context->cookie->showProducts;
    		$context->cookie->showProducts =  $factor + 1;
    		for ($i = 0; $i < 7 ; $i++) {
    			$count = ($factor * 7)+$i;
    			$index = $count%$len;
    			$result_products[$i] = $products[$index];
    		}
    			
    	}else {
    		$context->cookie->showProducts = 0;
    		$len = count($products);
    		for ($i = 0; $i < 7 && $len > $i; $i++) {
    			$result_products[$i] = $products[$i];
    		}
    	}
